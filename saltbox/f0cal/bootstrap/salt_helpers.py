...
class SaltDaemon(object):
    ...
    def start(self):
        if self.running:
            return
        _cfg = self._config_obj.config_dir
        assert os.path.exists(os.path.join(_cfg, cls.CFG_FILENAME))
        subprocess.call([self.EXE, "--config-dir", _cfg, "--daemon"])
        self._running = True

    def stop(self):
        if not self.running:
            return
        pid = self._config_obj.daemon_pid
    ...
    @property
    def pid_file(self):
        return os.path.join(self.config_di)
...
class SaltConfig(object):
    ...
    def __init__(self, root_dir, cleanup=None):
        ...
        self._config_dir = os.path.join(root_dir, "etc", "salt")
        self._minion_config = None
        self._minion_config_path = os.path.join(self._config_dir, "minion")
        self._master_config = None
        self._master_config_path = os.path.join(self._config_dir, "master")
